...
object Killer {
  ...
  def apply(serverList: ListBuffer[ListBuffer[ActorRef[Server.Command]]], supervisorLength: Int, numSupervisors: Int, killsPerSecond: Int, startIndex: Int, burstTime: Int = 0): Behavior[KillerTask] = 
    Behaviors.setup(context => new Killer(context, serverList, supervisorLength, numSupervisors, killsPerSecond, startIndex, burstTime))
}

class Killer(context: ActorContext[Killer.KillerTask], serverList: ListBuffer[ListBuffer[ActorRef[Server.Command]]], 
            supervisorLength: Int, numSupervisors: Int, killsPerSecond: Int, startIndex: Int, burstTime: Int = 0) extends AbstractBehavior[Killer.KillerTask](context) {
  import Killer._
    ...
    msg match {
      case KillerTask(mode) =>
        mode match {
          ...
          case "stairs" =>
            curTask = context.system.scheduler.scheduleOnce(burstTime*5.milli, (() => context.self ! KillerTask("stairs")))
            for (i <- 1 to kps) {
...
object InjectorMain {
  ...
  case class InjectorParams(nfails: Int, sender: ListBuffer[ListBuffer[ActorRef[Server.Command]]], injectionRate: Int) extends Command with CborSerializable
...
class InjectorMain(context: ActorContext[InjectorMain.Command], mode: String) extends AbstractBehavior[InjectorMain.Command](context) {
  ...
  override def onMessage(msg: Command): Behavior[Command] = 
    msg match {
      case InjectorParams(nfails, serverList, injectionRate) =>
        Thread.sleep(1000)
        ...
        mode match {
          ...
          case "stairs" =>
            Thread.sleep( 50*(injectionRate+20))
            var parts = serverList.grouped(spk)
            for (part <- parts) {
              var killer = context.spawnAnonymous(Killer(part, 50, spk, maxKills, 0, 5*injectionRate+100))
              killerList += killer
            }
            for (k <- killerList) {
